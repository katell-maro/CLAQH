<div class="footer">
  <img class="logo_part" src="../IMG/AEAG.png" alt="L'Agence de l'eau Adour Garonne" width=100px>
  <img class="logo_part" src="../IMG/irstea.png" alt="L'IRSTEA" width=100px>
  <img class="logo_part" src="../IMG/epoc.jpg" alt="EPOC" width=100px>
  <img class="logo_part" src="../IMG/ensc.PNG" alt="l'Ecole Nationale Supérieure de Cognitique" width=100px>
  <img class="logo_part" src="../IMG/labex_cote.jpg" alt="le labex cote" width=100px>
  <img class="logo_part" src="../IMG/universite_bordeaux.jpg" alt="L'Université de Bordeaux" width=100px>
  <img class="logo_part" src="../IMG/universite_pau.png" alt="L'Université de Pau et des pays de l’Adour" width=100px>
</div>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
<script src="../bootstrap/js/bootstrap.min.js"></script>
